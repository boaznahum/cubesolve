...
def _handle_face_slice_rotate_by_drag(window: AppWindow, x, y, dx, dy):
    ...
    vs.logger.log_lazy(CubeLogger.verbose_level(mouse_debug), lambda: f"[DRAG] Handler: x={x}, y={y}, dx={dx}, dy={dy}")

    if _FACE_ROTATING_BY_MOUSE_MOUSE_ALG_IS_RUNNING:
        vs.logger.log_lazy(CubeLogger.verbose_level(mouse_debug), "[DRAG] Skipped - already running")
        return
    ...
    vs.logger.log_lazy(CubeLogger.verbose_level(mouse_debug), lambda: f"[DRAG] Data points: {n}/{_DRAG_VECTOR_DETECTION_DATA_LENGTH}")
    ...
    if not selected:
        vs.logger.log_lazy(CubeLogger.verbose_level(mouse_debug), lambda: f"Mouse drag: Didn't find selected element: {x=}, {y=})")
        return
    ...
    vs.logger.log_lazy(CubeLogger.verbose_level(mouse_debug), lambda: f"[ROTATE] Selected: face={slice_edge.face.name}, left_to_right={left_to_right}, left_to_top={left_to_top}")
    ...
    vs.logger.log_lazy(CubeLogger.verbose_level(mouse_debug), lambda: f"[ROTATE] part={type(part).__name__}, d_vector={d_vector}, ltr={on_left_to_right:.2f}, ltt={on_left_to_top:.2f}")
    ...
    try:
        ...
        if alg:
            if inv:
                alg = alg.inv()
            vs.logger.log_lazy(CubeLogger.verbose_level(mouse_debug), lambda: f"[ROTATE] Playing: {alg}")
            _play(window, alg)
        else:
            vs.logger.log_lazy(CubeLogger.verbose_level(mouse_debug), "[ROTATE] No alg to play")
...
def _play(window: AppWindow, alg: Alg):
    ...
    vs.logger.log_lazy(CubeLogger.verbose_level(vs.config.input_mouse_debug), lambda: f"[PLAY] Playing {alg}")
...
def _handle_selected_slice(window: AppWindow, slice_face: PartEdge, inv: bool):
    ...
    if slice_face:
        ...
        if isinstance(part, Edge):
            ...
            slice_alg_base: SliceAlg | None = None
            ...
            if slice_alg_base:
                ...
                slice_alg = slice_alg_base[index + 1]  # index start from 1
...
def _get_selected_slice(vs, window, x, y) -> Tuple[PartEdge, np.ndarray, np.ndarray] | None:
    ...
    vs.logger.log_lazy(CubeLogger.verbose_level(mouse_debug), lambda: f"[PICK] screen=({x}, {y}), window=({window.width}, {window.height})")
    ...
    if result is None:
        vs.logger.log_lazy(CubeLogger.verbose_level(mouse_debug), "[PICK] No hit")
        return None

    part_edge, right_dir, up_dir = result
    vs.logger.log_lazy(CubeLogger.verbose_level(mouse_debug), lambda: f"[PICK] Hit: {part_edge.face.name} part={type(part_edge.parent.parent).__name__}")
