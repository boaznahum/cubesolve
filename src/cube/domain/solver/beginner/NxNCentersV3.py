...
=============================================================================
PRESERVE 3x3 STATE MODE (preserve_3x3_state=True)
=============================================================================

When preserve_3x3_state=True (for cage method):
- Setup moves are tracked and UNDONE to preserve paired edges and corners
- Slice swap optimizations are DISABLED (they break corners)
- Face swap optimizations are DISABLED (they break corners)

When preserve_3x3_state=False (for reduction method):
- Setup moves are NOT undone (faster, fewer moves)
- Slice and face swap optimizations are enabled (from config)
- Breaks paired edges - only use BEFORE edge pairing!

=============================================================================
WHY SETUP MOVES BREAK THE CAGE (PAIRED EDGES)
=============================================================================

The core commutator in _block_communicator uses SINGLE F rotations:
...
The COMMUTATOR ITSELF is balanced (F and F' cancel out), so corners
and edge pairing are preserved by the commutator. But SETUP MOVES
(source face rotations, F' to convert row to column) are NOT balanced
and must be explicitly undone.

For reduction method: Setup moves not undone - edges not yet paired
For cage method: Setup moves UNDONE - preserves paired edges!
...
class NxNCentersV3(SolverElement):
    """
    Solves center pieces on NxN cubes with optional 3x3 state preservation.

    This is a unified version that can operate in two modes:
    - preserve_3x3_state=False: For reduction method (centers before edges)
    - preserve_3x3_state=True: For cage method (preserves paired edges)
    ...
    2. Use three strategies (in order of efficiency):
       a. Swap complete slices (entire rows/columns) - disabled when preserving
       b. Move blocks of matching pieces
       c. Move individual pieces via commutators

    PRESERVE 3x3 STATE MODE:
    ========================

    When preserve_3x3_state=True:
    - _OPTIMIZE_BIG_CUBE_CENTERS_SEARCH_COMPLETE_SLICES = False
    - _OPTIMIZE_ODD_CUBE_CENTERS_SWITCH_CENTERS = False
    - All setup moves are tracked and UNDONE after each algorithm
    - The commutator itself is BALANCED (2 F + 2 F' = 0)
    - Only the setup moves break pairing, so we undo them
    ...
    FOR EVEN CUBES:
    ===============
    - Uses FaceTracker to establish color mapping
    """

    def __init__(self, slv: SolverElementsProvider, preserve_3x3_state: bool = False) -> None:
        """
        Initialize the NxN centers solver.

        :param slv: The solver elements provider
        :param preserve_3x3_state: If True, undo setup moves to preserve
                                   paired edges and solved corners (cage method).
                                   If False, don't undo setup moves (reduction method).
        """
        super().__init__(slv)
        ...
        if preserve_3x3_state:
            ...
            # Reduction method: use optimizations from config
    ...
    def solve(self):
        ...
        PRESERVES 3x3 SOLUTION: Only if preserve_3x3_state=True
        =======================================================
        When preserve_3x3_state=True, this method preserves paired edges
        and solved corners. All internal methods either:
    ...
    def _solve(self) -> None:
        ...
        PRESERVES 3x3 SOLUTION: Only if preserve_3x3_state=True
        =======================================================
        Delegates to __do_center which preserves 3x3 solution when enabled.
        ...
        - F rotations - to set up commutators (undone when preserve_3x3_state=True)
        """
        cube = self.cube
    ...
    def _do_faces(self, faces: Sequence[FaceTracker], minimal_bring_one_color, use_back_too: bool) -> bool:
        ...
        for f in faces:
            ...
            if self._do_center(f, minimal_bring_one_color, use_back_too):
                ...
                if len(faces) == 6:
                    self._asserts_is_boy(faces)
            # if NxNCentersV3.work_on_b or not work_done:
            #     break
    ...
    def __do_center(self, face_loc: FaceTracker, minimal_bring_one_color: bool, use_back_too: bool) -> bool:
        ...
        with self.ann.annotate(h2=f"{color2long(face_loc.color).value} face"):
            ...
            # Verify FaceTracker correctly tracked the face to front position
            # Note: Can't assert cube.front.color == color on even cubes since
            # center pieces don't define face color until centers are solved
            assert face_loc.face is cube.front
            ...
            if any(self._has_color_on_face(f, color) for f in cube.front.adjusted_faces()):
                if self._preserve_3x3_state:
                    ...
                    # =============================================================
                    # REDUCTION METHOD: NO UNDO NEEDED
                    # =============================================================
                    # Setup rotations are not undone - edges not yet paired.
                    # =============================================================
    ...
    def _do_center_from_face(self, face: Face, minimal_bring_one_color, color: Color, source_face: Face) -> bool:
        ...
        PRESERVES 3x3 SOLUTION: Only if preserve_3x3_state=True
        =======================================================
        - _do_complete_slices: DISABLED when preserve_3x3_state=True
        - _do_blocks: Calls _block_communicator which preserves 3x3 when enabled
        - Single piece loop: Calls _block_communicator which preserves 3x3 when enabled
    ...
    def _do_complete_slices(self, color, face, source_face) -> bool:
        ...
        PRESERVES 3x3 SOLUTION: NO - DISABLED when preserve_3x3_state=True!
        ===================================================================
        This method calls _swap_slice which uses U2/B2 that breaks corners.
        It is DISABLED when preserve_3x3_state=True via:
            _OPTIMIZE_BIG_CUBE_CENTERS_SEARCH_COMPLETE_SLICES = False
        """
        work_done = False
    ...
    def _swap_slice(self, target_slice: _CompleteSlice,
        ...
        PRESERVES 3x3 SOLUTION: Only if preserve_3x3_state=True
        =======================================================
        When preserve_3x3_state=True, setup moves (F', source rotation)
        are tracked and UNDONE at the end.

        When preserve_3x3_state=False, setup moves are NOT undone.
        ...
        1. NORMALIZE TARGET TO VERTICAL
           ...
           - When preserve_3x3_state=True, this F' is UNDONE at the end

        2. ROTATE SOURCE TO ALIGN
           - Rotate source face so its slice aligns with target column
           - When preserve_3x3_state=True, this is UNDONE at the end
        ...
        if target_slice.is_row:
            ...
            did_f_prime_setup = True  # Remember to undo if preserving
        else:
            # column
    ...
    def _do_blocks(self, color, face, source_face):
        ...
        and undoes its source face setup rotation when preserving.
        """
        work_done = False
    ...
    def _bring_face_up_preserve_front(self, face) -> algs.Alg:
        ...
        =================================================================
        These rotations MOVE CORNERS AND EDGES. When preserve_3x3_state=True,
        we must UNDO them after processing each face. By returning the alg
        played, the caller can accumulate all rotations and undo with alg.prime.
    ...
    def _block_communicator(self,
        ...
        PRESERVES 3x3 SOLUTION: Only if preserve_3x3_state=True
        =======================================================
        - The commutator itself is BALANCED: 2xF + 2xF' = net 0 rotation
        - Corners return to original positions after the 8 moves
        - When preserve_3x3_state=True, source face setup rotation (n_rotate)
          is UNDONE at the end
        ...
        This executes an 8-move commutator sequence:
            ...
            cum = [
                ...
                F (or F'),            # Step 2: Rotate front face
                ...
                F (or F'),            # Step 6: Rotate front face
        ...
        The commutator is BALANCED (F and F' cancel out), so corners
        and edges return to their original positions. Only centers move.
        """
        cube: Cube = face.cube
        assert face is cube.front
        ...
        # For back face, need double rotation (180 deg) to reach it
        if is_back:
            rotate_mul = 2
        else:
            rotate_mul = 1
        ...
        # =====================================================================
        # THE 8-MOVE COMMUTATOR - BALANCED!
        # =====================================================================
        #
        ...
        #
        # The F rotations (on_front_rotate) are BALANCED (2 F + 2 F' = 0)
        # so corners and edges return to original positions.
        # =====================================================================
        cum = [
            rotate_on_cell.prime * rotate_mul,   # Step 1: M' - move target col to source face
            on_front_rotate,                      # Step 2: F  - rotate front
            rotate_on_second.prime * rotate_mul, # Step 3: M' - move buffer col to source face
            on_front_rotate.prime,               # Step 4: F' - undo front rotation
            rotate_on_cell * rotate_mul,         # Step 5: M  - return target col
            on_front_rotate,                      # Step 6: F  - rotate front again
            rotate_on_second * rotate_mul,       # Step 7: M  - return buffer col
            on_front_rotate.prime                # Step 8: F' - final undo
        ]
    ...
    def _count_colors_on_block(color: Color, source_face: Face, rc1: Tuple[int, int], rc2: Tuple[int, int],
        ...
        n, _ = NxNCentersV3._count_colors_on_block_and_tracker(color, source_face, rc1, rc2, ignore_if_back)
