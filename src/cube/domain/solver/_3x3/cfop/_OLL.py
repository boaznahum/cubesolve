...
from cube.domain.exceptions import EvenCubeEdgeParityException, InternalSWError
...
class OLL(StepSolver):
    ...
    def _do_oll(self) -> None:
        ...
        for r in range(4):
            ...
            self._logger.log_lazy(logging.DEBUG, lambda: f"Found state after {r} rotations:\n{state}")
        ...
        self._logger.log_lazy(logging.DEBUG, lambda: f"Found OLL alg '{description}' {alg}")
    ...
    def _check_edge_parity(self) -> None:
        ...
        if n_edges not in [0, 2, 4]:
            if self.cube.n_slices % 2 == 0 or self.cube.is_even_cube_shadow:
                self._logger.log_lazy(logging.DEBUG, lambda: f"OLL: Edge parity detected ({n_edges} edges matching)")
    ...
    def _get_state_alg_raw(self, state: str) -> Tuple[str, Alg | str] | None:
        ...
        self._logger.log_lazy(logging.DEBUG, lambda: f"Comparing state:{state2}")
