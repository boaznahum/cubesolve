...
from cube.domain.tracker._face_trackers import (
    FaceTracker,
    MarkedFaceTracker,
    SimpleFaceTracker,
...
class NxNCentersFaceTrackers(SolverHelper):
    ...
    def _create_tracker_on_face(self, parent_container: FacesTrackerHolder, face: Face, color: Color) -> MarkedFaceTracker:
        ...
        _helper.find_and_track_slice(face, key, color)

        return MarkedFaceTracker(self.cube, parent_container, color, key)
    ...
    def _track_two_last_even_cube(self, parent_container: FacesTrackerHolder, four_first: Sequence[FaceTracker]) -> Tuple[FaceTracker, FaceTracker]:
        ...
        3. Check if this creates a valid color scheme using CubeColorScheme.same()
        ...
        We try one, verify with CubeColorScheme.same(), and use the other if invalid.
