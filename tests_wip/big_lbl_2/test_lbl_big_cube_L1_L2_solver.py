...
class TestLBLBigCubeSolver:
    ...
    def test_lbl_l1_then_l2_slices(
        self,
        cube_size: int,
        scramble_name: str,
        scramble_seed: int,
        ...
        assert solver.is_solved_phase(SolveStep.L1), (
            f"L2 not solved (size={cube_size}, scramble={scramble_name})"
        )

        solver.solve(what=SolveStep.LBL_L2_SLICES, debug=False, animation=False)

        assert solver.is_solved_phase(SolveStep.LBL_L2_SLICES), (
            f"L2 centers not solved (size={cube_size}, scramble={scramble_name})"
        )
