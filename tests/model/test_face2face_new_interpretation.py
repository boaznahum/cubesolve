"""
Test Face2FaceTranslator with the REQUIRED interpretation.

REQUIRED NAMING (what we actually need):
...
def verify_with_required_interpretation(
    cube: Cube,
    target_face: Face,  # Was: source_face - where content ARRIVES
    source_face: Face,  # Was: dest_face - where content COMES FROM
    target_coord: CenterSliceIndex,  # Was: coord/source_coord
) -> None:
    """
    Verify using REQUIRED interpretation.
    ...
    # Call translate with CURRENT parameter order (which maps to required names)
    # Current: translate(source_face, dest_face, source_coord)
    # Required: translate(target_face, source_face, target_coord)
    # So we pass: target_face as "source_face", source_face as "dest_face"
    result = Face2FaceTranslator.translate(target_face, source_face, target_coord)

    # result.dest_coord is actually source_coord (where content comes from)
    source_coord = result.dest_coord
    ...
    assert check_slice.edge.c_attributes.get("test_marker") == marker_value, (
        f"FAILED with required interpretation!\n"
...
class TestRequiredInterpretation:
    """Test that the implementation works with required interpretation."""

    @pytest.mark.parametrize("cube_size", [3, 5])
    def test_whole_cube_alg_required_interpretation(self, cube_size: int) -> None:
        """Test whole-cube algorithm with required naming."""
        ...
        for target_face in cube.faces:
            for source_face in target_face.others_faces:
                for center_slice in target_face.center.all_slices:
                    ...
                    verify_with_required_interpretation(
    ...
    def test_slice_alg_required_interpretation(self, cube_size: int) -> None:
        """Test slice algorithm with required naming."""
        cube = Cube(cube_size, sp=_test_sp)
